...
azdev-tools.cjs CLI contract:
  ...
  node ~/.claude/azdev-skill/bin/azdev-tools.cjs save-config --org "<org>" --project "<project>" --pat "<pat>" --cwd $CWD
...
6. **Save credentials:**
   Run `node ~/.claude/azdev-skill/bin/azdev-tools.cjs save-config --org "<org>" --project "<project>" --pat "<pat>" --cwd $CWD`
