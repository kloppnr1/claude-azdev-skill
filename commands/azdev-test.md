...
1. **Check azdev-tools.cjs exists:**
   Check that `~/.claude/bin/azdev-tools.cjs` exists before running.
   If it does not exist: Tell user "Azure DevOps tools not installed. Check that ~/.claude/bin/azdev-tools.cjs exists."
   Stop.
